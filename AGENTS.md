...
노트, 차량, 정보, 홈, 춘천 페이지의 본문·목록·표는 같은 `.wrap` 폭을 쓴다. 최대 폭은 1040px이다. 문단이나 목록만 `ch`로 좁히지 않는다. 글을 쓸 때도 신문 칼럼처럼 짧게 끊지 말고, 표와 같은 가로폭에 맞춰 쓴다.

다섯 열 이상인 표는 먼저 네 열 이하로 줄인다. 역할·배경 설명은 표 아래 문단으로 옮긴다. 태그명, 프로그램명, Routine명은 임의로 줄바꿈하거나 축약하지 않는다. 좁은 화면에서는 다열 표를 작은 글자로 짜내지 않고 표 안에서 가로로 스크롤한다. 새 노트를 작성하거나 기존 노트를 고칠 때도 이 규칙을 적용한다.
