...
다음 그림은 [L5X Ladder Studio](https://alzza.github.io/l5x-ld-studio/?v=e136d3b)의 **고정 · 노트용** 배율로 넣었다. 렁이 짧다고 가로로 늘리지 않아서 접점과 코일 크기가 그림마다 달라지지 않는다. 웹에서는 Rung 번호와 파서 상태를 그림 위에 두고, SVG 안에는 번호를 넣지 않았다. 모든 그림의 파서 경고는 0건이었다.
