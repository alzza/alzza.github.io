...
export async function listVehicleReports(): Promise<VehicleReportItem[]> {
  ...
  const legacy = reports.map((report) => ({
  ...
  const data = dataReports.map((report) => ({
    href: `/vehicle/${report.id}/`, title: report.data.title, date: report.data.date, excerpt: report.data.excerpt,
    weekStart: report.data.week_start, weekEnd: report.data.week_end, riskLevel: report.data.risk_level,
    distanceKm: report.data.distance_km, chargingKwh: report.data.charging_kwh, capacityKwh: report.data.capacity_kwh,
    degradationPercent: report.data.degradation_percent,
    dailyMetrics: report.data.daily_metrics.map((item) => ({ date: item.date, distanceKm: item.distance_km, chargingKwh: item.charging_kwh })),
    capacityTrend: report.data.capacity_trend.map((item) => ({ month: item.month, capacityKwh: item.capacity_kwh })),
    accelerationWeek: { thresholdKw: report.data.acceleration_week.threshold_kw, secondaryThresholdKw: report.data.acceleration_week.secondary_threshold_kw, eventCount: report.data.acceleration_week.event_count, secondaryEventCount: report.data.acceleration_week.secondary_event_count, maxDischargePowerKw: report.data.acceleration_week.max_discharge_power_kw, avgPeakPowerSpeedKmh: report.data.acceleration_week.avg_peak_power_speed_kmh, maxPeakPowerSpeedKmh: report.data.acceleration_week.max_peak_power_speed_kmh, coveredDistanceKm: report.data.acceleration_week.covered_distance_km, eventsPer1000Km: report.data.acceleration_week.events_per_1000km, dataQuality: report.data.acceleration_week.data_quality },
    accelerationTrend: report.data.acceleration_trend.map((item) => ({ month: item.month, events180Kw: item.events_180kw, events200Kw: item.events_200kw, avgPeakPowerSpeedKmh: item.avg_peak_power_speed_kmh, maxPeakPowerSpeedKmh: item.max_peak_power_speed_kmh, coveredDistanceKm: item.covered_distance_km, eventsPer1000Km: item.events_per_1000km })),
    tags: report.data.tags,
  }));
  const merged = new Map([...legacy, ...data].map((item) => [item.href, item]));
  return [...merged.values()].sort((a, b) => (a.weekStart < b.weekStart ? 1 : -1));
}
